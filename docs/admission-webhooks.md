...
The chain fires inside `Wiki::write_page`, **after** the markdown is
parsed and sanitised but **before** the atomic write and store upsert.
A mutation applied by a webhook propagates to both the on-disk markdown
file and the SQLite row in a single atomic step (see
...
`WebhookConfig::name` and short-circuits those hooks for that write.
Without this, you get infinite recursion (engine → webhook → engine →
webhook → ...). The header propagates only for the single re-entrant
...
```bash
AI_MEMORY_ADMISSION_WEBHOOKS__0__NAME=contributors
AI_MEMORY_ADMISSION_WEBHOOKS__0__URL=http://contributors.memory.svc.cluster.local:8080/enrich
AI_MEMORY_ADMISSION_WEBHOOKS__0__TIMEOUT_MS=2000
AI_MEMORY_ADMISSION_WEBHOOKS__0__FAILURE_POLICY=ignore
AI_MEMORY_ADMISSION_WEBHOOKS__0__EVENTS=write_page,consolidate
```
